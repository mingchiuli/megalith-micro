...
public class CacheAspect {
  ...
  public CacheAspect(
      ...
      com.github.benmanes.caffeine.cache.Cache<@NonNull String, Object> localCache,
  ...
  private Object handleCacheMiss(ProceedingJoinPoint pjp, Method method, String cacheKey)
      throws Throwable {
    ...
    try {
      String remoteCacheStr = getRemoteCache(cacheKey);
      if (StringUtils.hasLength(remoteCacheStr)) {
        return parseRemoteCache(
            remoteCacheStr,
            jsonMapper.getTypeFactory().constructType(method.getGenericReturnType()),
            cacheKey);
      }
