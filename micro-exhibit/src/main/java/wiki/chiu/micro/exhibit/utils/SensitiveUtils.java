...
public class SensitiveUtils {
    ...
    public static BlogExhibitDto deal(
        List<SensitiveContentRpcVo> sensitiveWords, BlogExhibitDto blog) {
        String title = processSensitiveContent(sensitiveWords, blog.title(), SensitiveTypeEnum.TITLE);
        String description =
            processSensitiveContent(sensitiveWords, blog.description(), SensitiveTypeEnum.DESCRIPTION);
        String content =
            processSensitiveContent(sensitiveWords, blog.content(), SensitiveTypeEnum.CONTENT);

        return BlogExhibitDtoConvertor.convert(blog, title, description, content);
    }

    public static BlogDescriptionDto deal(
        List<SensitiveContentRpcVo> sensitiveWords, BlogDescriptionDto blog) {
        String title = processSensitiveContent(sensitiveWords, blog.title(), SensitiveTypeEnum.TITLE);
        String description =
            processSensitiveContent(sensitiveWords, blog.description(), SensitiveTypeEnum.DESCRIPTION);
    ...
    private static String processSensitiveContent(
        List<SensitiveContentRpcVo> sensitiveWords, String content, SensitiveTypeEnum type) {
        List<SensitiveContentRpcVo> filteredWords =
            sensitiveWords.stream().filter(item -> type.getCode().equals(item.type())).toList();
