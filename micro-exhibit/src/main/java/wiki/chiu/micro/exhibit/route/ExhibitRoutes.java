...
public class ExhibitRoutes {
    ...
    public static RouterFunction<ServerResponse> routes(BlogExhibitHttpHandler handler) {
        return withDefaultErrorHandling(route()
                ...
                .GET("/public/blog/secret/{blogId}", handler::getLockedBlog)
                .GET("/public/blog/token/{blogId}", handler::checkReadToken)
                .GET("/public/blog/stat", handler::getVisitStatistics)
                .GET("/public/blog/scores", handler::getScoreBlogs), log)
                .build();
    }
}
