...
pub(crate) async fn request_raw(
    ...
    let request = parse_request(url, method, headers)
        .await?
        .body(body)
...
mod tests {
    ...
    #[test]
    fn extracts_authority_for_the_host_header() {
        let uri: Uri = "http://localhost:8080/path".parse().unwrap();
        assert_eq!(parse_host(&uri).unwrap(), "localhost:8080");
    }

    #[test]
    fn rejects_a_relative_control_plane_uri() {
        let uri: Uri = "/auth/route".parse().unwrap();
        assert!(matches!(parse_host(&uri), Err(ClientError::Request(_))));
    }
}
