...
- ✅ 无 `const enum`、namespace、自定义 transformer、旧式 `moduleResolution`（app=`Bundler`，node/server=`NodeNext`/`node16`）
- ✅ tsconfig 全链（含 `@vue/tsconfig`、`@tsconfig/node24`）不含任何 TS7 移除的选项（兼容性扫描零命中）
...
1. `typescript` 改为指向 JS API 垫片（供 vue-tsc + 编辑器语言服务）：
   ...
   ```
2. 新增原生编译器（用于 `build:node`、`type-check:node`）：
   ```bash
...
1. 升级依赖（按上面双装或直装 TS7）
2. `npm run type-check` —— 修 `strictBuiltinIteratorReturn` 等新报错
3. `npm run build:node` —— 验证原生 `tsc` emit（本项目 `declaration: false`，风险低）
4. `npm run check` —— 全量 lint + format + type-check + test + build + SSR/OTel smoke
